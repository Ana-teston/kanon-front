...
import {
  ...
  setSearchField,
  setSearchResults,
} from '../../state/actions/gamesAction'
...
const GamesPage: React.FC = () => {
  ...
  const searchResults = useSelector(
    (state: RootState) => state.games.searchResults,
  )
    ...
    const fetchGames = async () => {
      try {
        ...
        if (response && Array.isArray(response.games)) {
          ...
          console.error(
            'Invalid data structure. Expected an object with a "games" array property, but received:',
            response,
          )
        }
      } catch (error) {
